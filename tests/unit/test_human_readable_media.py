...
from qti_package_maker.engines.human_readable import engine_class
...
#============================================
def test_save_package_keeps_readable_name_alt_and_source(
	...
	outfile = str(tmp_path / "human-out.html")
	...
	assert "alt: fig" in written_text
	assert "source: images/foo.png" in written_text
	# no file is copied or embedded; the source path stays a plain reference
...
def test_save_package_ignores_data_src_decoy(tmp_path: pathlib.Path) -> None:
	...
	engine = engine_class.EngineClass("sample", verbose=False)
	outfile = str(tmp_path / "human-out-datasrc.html")
	engine.save_package(bank, outfile=outfile)
	written_text = pathlib.Path(outfile).read_text(encoding="utf-8")
	assert "<img" not in written_text
	assert "[image: figure.png]" in written_text
	assert "source: images/figure.png" in written_text
