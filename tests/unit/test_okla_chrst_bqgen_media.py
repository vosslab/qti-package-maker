...
from qti_package_maker.engines.okla_chrst_bqgen import engine_class
...
def test_save_package_emits_placeholder_and_warns(
	...
	outfile = str(tmp_path / "okla-out.txt")
...
def test_save_package_ignores_data_src_decoy(tmp_path: pathlib.Path) -> None:
	...
	engine = engine_class.EngineClass("sample", verbose=False)
	outfile = str(tmp_path / "okla-out-datasrc.txt")
	engine.save_package(bank, outfile=outfile)
	written_text = pathlib.Path(outfile).read_text(encoding="utf-8")
...
#============================================
def test_placeholder_round_trips_as_plain_text(tmp_path: pathlib.Path) -> None:
	# okla_chrst_bqgen has no image markup channel, so the placeholder is
	# expected to survive a read-write cycle only as ordinary inline text
	# (not as a re-resolved MediaAsset reference).
	bank = _make_bank_with_image(tmp_path)
	engine = engine_class.EngineClass("sample", verbose=False)
	outfile = str(tmp_path / "okla-out.txt")
	engine.save_package(bank, outfile=outfile)
	roundtripped_bank = engine.read_items_from_file(outfile, allow_mixed=True)
	roundtripped_item = next(iter(roundtripped_bank.items_dict.values()))
	assert "[image: foo.png]" in roundtripped_item.question_text
